...
- Completed `VNT-301`: documented verified `POST /messaging/main/conversations/[CONVERSATION_ID]/replies` on `api.vinted.fr`, implemented scoped plain-text `VintedMessagingClient.sendMessage`, fixed `is_personal_data_sharing_check_skipped=false`, kept `photo_temp_uuids=null`, disabled request/response diagnostic previews, added sanitized fixtures/tests, and marked `Send message` as `IMPLEMENTED`.
